...
else
{
    (function()
    {
        ...
        // 🚀 针对 Electron 桌面端的并发加载与延迟加载图形库优化
        if (mxIsElectron)
        {
            var scripts = [
                'js/PreConfig.js',
            ...
            function onLastLoaded()
            {
                ...
                mxWinLoaded = true; // 🚀 不等 window.load，直接就绪
                checkAllLoaded();
                
                // 🚀 主界面就绪 50ms 后，静默延迟加载 12.2MB 的非核心图形和插件库，不阻塞首屏显示
                setTimeout(function() {
                    ...
                    backgroundScripts.forEach(function(src) {
                        ...
                        if (t != null)
                        {
                            t.parentNode.insertBefore(bgScript, t);
                        }
                        else
                        {
            ...
            for (var i = 0; i < scripts.length; i++)
            {
                ...
                s.src = scripts[i];
                s.async = false; // 并行下载，但严格按插入顺序执行

                if (i === scripts.length - 1)
                {
                    s.onload = onLastLoaded;
                }

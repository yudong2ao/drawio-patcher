...
(async function()
{
	...
	// 强行将主题限定为经典主题
	Editor.themes = [];
	mxSettings.getUi = function()
	{
		return 'kennedy';
	};

	// 桌面离线按需动态加载 Mermaid/ELK
	var editorUiLoadMermaid = EditorUi.prototype.loadMermaid;
	EditorUi.prototype.loadMermaid = function(success, error)
	{
		if (mxIsElectron && (this.isOffline(true) || typeof mxMermaidToDrawio === 'undefined'))
		{
			...
			if (typeof mxMermaidToDrawio === 'undefined' && !this.loadingMermaid)
			{
				this.loadingMermaid = true;
				mxscript('js/elk/drawio-elk.min.js', mxUtils.bind(this, function()
				{
		...
		{
			editorUiLoadMermaid.apply(this, arguments);
		}
	};
	...
	EditorUi.prototype.init = async function()
	{
		...
		console.log('[Renderer Log] === ElectronApp.js: EditorUi.prototype.init starting ===');
		var isWindows = mxClient.IS_WIN || (navigator.userAgent.indexOf('Windows') > -1) || (navigator.appVersion.indexOf('Win') > -1);
		console.log('[Renderer Log] mxClient.IS_WIN:', mxClient.IS_WIN, 'isWindows determined:', isWindows);

		// 仅在 Windows 桌面端注入自定义标题栏按钮和拖拽样式
		if (isWindows)
		{
			// 1. 注入 CSS 样式
			...
			style.innerHTML = `
				.geMenubarContainer {
					...
					padding-right: 150px !important; /* 预留右侧控制按钮空间 */
				}
				.geMenubar, .geMenubar *, .geButtonContainer, .geButtonContainer * {
				...
				.window-controls-container {
					...
					top: 0;
					right: 0;
					height: 36px; /* 默认模式下高度设为 36px，精致美观 */
					display: flex;
					align-items: center;
					z-index: 299999; /* 超高层级防止被遮挡 */
					-webkit-app-region: no-drag;
					padding-right: 4px;
				...
				.window-control-btn {
					width: 46px;
					height: 100%; /* 高度跟随容器自适应 */
					display: flex;
					justify-content: center;
					align-items: center;
					cursor: pointer;
					transition: background-color 0.15s, color 0.15s;
					color: #333333; /* 默认浅色主题图标颜色 */
				}
				...
				/* 折叠的紧凑模式下自动将容器高度限制为 30px */
				.geCompactMode .window-controls-container {
					height: 30px;
				}

				/* 暗黑模式下切换颜色 */
				.geDarkMode .window-control-btn {
					color: #cccccc;
				}
				.geDarkMode .window-control-btn:hover {
			...
			btnMin.className = 'window-control-btn min-btn';
			btnMin.title = mxResources.get('minimize') || 'Minimize';
			btnMin.innerHTML = '<svg viewBox="0 0 10 1"><path d="M0 0h10v1H0z"/></svg>';
			btnMin.addEventListener('click', function()
			{
				electron.request({action: 'windowAction', method: 'minimize'}, function() {});
			});

			// 最大化按钮
			var btnMax = document.createElement('div');
			btnMax.className = 'window-control-btn max-btn';
			btnMax.title = mxResources.get('maximize') || 'Maximize';
			var maxSvg = '<svg viewBox="0 0 10 10"><path d="M0 0v10h10V0H0zm9 9H1V1h8v8z"/></svg>';
			var restoreSvg = '<svg viewBox="0 0 10 10"><path d="M2 0v2H0v8h8V8h2V0H2zm5 9H1V3h6v6zm2-2H8V2H3V1h6v6z"/></svg>';
			btnMax.innerHTML = maxSvg;

			var updateMaxIcon = function(isMax)
			{
				if (isMax)
				{
					btnMax.innerHTML = restoreSvg;
					btnMax.title = mxResources.get('restore') || 'Restore';
				}
				else
				{
					btnMax.innerHTML = maxSvg;
			...
			// 初始化最大化状态
			electron.request({action: 'windowAction', method: 'isMaximized'}, function(isMax)
			{
				updateMaxIcon(isMax);
			});
			...
			btnMax.addEventListener('click', function()
			{
				electron.request({action: 'windowAction', method: 'isMaximized'}, function(isMax)
				{
					if (isMax)
					{
						electron.request({action: 'windowAction', method: 'unmaximize'}, function() {});
					}
					else
					{
						electron.request({action: 'windowAction', method: 'maximize'}, function() {});
					}
				});
			});

			// 监听最大化状态变化
			electron.registerMsgListener('maximize', function()
			{
				updateMaxIcon(true);
			});
			electron.registerMsgListener('unmaximize', function()
			{
				updateMaxIcon(false);
			});
			...
			btnClose.className = 'window-control-btn close-btn';
			btnClose.title = mxResources.get('close') || 'Close';
			btnClose.innerHTML = '<svg viewBox="0 0 10 10"><path d="M10 .707L9.293 0 5 4.293.707 0 0 .707 4.293 5 0 9.293l.707.707L5 5.707 9.293 10l.707-.707L5.707 5z"/></svg>';
			btnClose.addEventListener('click', function()
			{
				electron.request({action: 'windowAction', method: 'close'}, function() {});
			});

			controlsContainer.appendChild(btnMin);
			controlsContainer.appendChild(btnMax);
			controlsContainer.appendChild(btnClose);
			
			// 直接追加到 document.body 最外层，确保 z-index 起效且不受 menubar overflow:hidden 限制
			document.body.appendChild(controlsContainer);
			console.log('[Renderer Log] controlsContainer directly appended to document.body');
		}
